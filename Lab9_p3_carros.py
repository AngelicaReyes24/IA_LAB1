...
# Cargar y Preparar el Dataset de datos de automóviles
def load_car_data(file_path):
    # Cargar el archivo CSV
    df = pd.read_csv(file_path, encoding='ISO-8859-1')
    
    # Seleccionar las características (X) y la etiqueta (y)
    X = df[['city_mpg', 'combination_mpg', 'cylinders', 'displacement', 
            'highway_mpg', 'year']].values.tolist()
    y = df['class'].tolist()
    
    return X, y

# Ruta del archivo de datos de automóviles
car_file_path = '/content/car_data.csv'
X_car, y_car = load_car_data(car_file_path)

# Función Principal para Ejecutar el Clasificador con Métodos de Validación
def main():
    classifier = EuclideanClassifier()
    
    # Hold-Out Validation con Dataset de Automóviles
    accuracy_hold_out, confusion_hold_out = hold_out(X_car, y_car, classifier)
    print("Hold-Out Validation Accuracy:", accuracy_hold_out)
    print("Hold-Out Confusion Matrix:", confusion_hold_out)
    
    # 10-Fold Cross-Validation con Dataset de Automóviles
    accuracy_k_fold, confusions_k_fold = k_fold_cross_validation(X_car, y_car, classifier, k=10)
    print("\n10-Fold Cross-Validation Accuracy:", accuracy_k_fold)
    
    # Leave-One-Out Cross-Validation con Dataset de Automóviles
    accuracy_loo, confusions_loo = leave_one_out_cross_validation(X_car, y_car, classifier)
