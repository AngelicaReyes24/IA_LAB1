...
# Cargar y Preparar el Dataset de patrones de sueño
def load_sleep_data(file_path):
    # Cargar el archivo CSV
    df = pd.read_csv(file_path, encoding='ISO-8859-1')
    
    # Seleccionar las características (X) y la etiqueta (y)
    X = df[['Age', 'Sleep_Duration', 'Study_Hours', 'Screen_Time', 'Caffeine_Intake',
            'Physical_Activity', 'Weekday_Sleep_Start', 'Weekend_Sleep_Start',
            'Weekday_Sleep_End', 'Weekend_Sleep_End']].values.tolist()
    y = df['Sleep_Quality'].tolist()
    
    return X, y

# Ruta del archivo de patrones de sueño
sleep_file_path = '/content/student_sleep_patterns.csv'
X_sleep, y_sleep = load_sleep_data(sleep_file_path)

# Función Principal para Ejecutar el Clasificador con Métodos de Validación
def main():
    classifier = EuclideanClassifier()
    
    # Hold-Out Validation con Dataset de Patrones de Sueño
    accuracy_hold_out, confusion_hold_out = hold_out(X_sleep, y_sleep, classifier)
    print("Hold-Out Validation Accuracy:", accuracy_hold_out)
    print("Hold-Out Confusion Matrix:", confusion_hold_out)
    
    # 10-Fold Cross-Validation con Dataset de Patrones de Sueño
    accuracy_k_fold, confusions_k_fold = k_fold_cross_validation(X_sleep, y_sleep, classifier, k=10)
    print("\n10-Fold Cross-Validation Accuracy:", accuracy_k_fold)
    
    # Leave-One-Out Cross-Validation con Dataset de Patrones de Sueño
    accuracy_loo, confusions_loo = leave_one_out_cross_validation(X_sleep, y_sleep, classifier)
